## What it is
Pi is what happens when you rethink harnesses from scratch. Starting as minimal as possible. Letting the very same AI agent that you're using for coding build and configure the harness on top of it. 

In a way, it democratizes the Claude Code / Codex CLI developer team and process. 

## References

- AI Engineer. (2026, April 16). [Building pi in a World of Slop - Mario Zechner](https://www.youtube.com/watch?v=RjfbvDXpFls&list=WL&index=2). YouTube video.
